...
interface Props {
  setIsQuestionNumberExceedingLimit: React.Dispatch<React.SetStateAction<boolean>>;
}

const QuestionNumberExceedingLimitAlertModal = (props: Props) => {
  const { setIsQuestionNumberExceedingLimit } = props;
  return (
    <div
      className={
      ...
      <div className={'flex flex-col gap-y-4 bg-white rounded-[32px] p-5'}>
        <div className={'flex flex-col gap-y-1'}>
          <div className={'text-h2 font-semibold text-black'}>문제 번호를 확인해주세요!</div>
          <div>
            <div className={'flex items-center text-h6'}>입력하신 번호가 최대 문제 갯수를 초과했습니다.</div>
          </div>
        </div>
      </div>
    </div>
...
export default QuestionNumberExceedingLimitAlertModal;
